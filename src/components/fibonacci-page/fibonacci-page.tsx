...
export const FibonacciPage: React.FC = () => {
  ...
  const [valueString, setValueString] = useState<Array<TFibonacci>>([]);
  ...
  const createArr = (str: string) => {
    ...
    for (let i = 0; i < str.length; i++) {
      let item: TFibonacci = { letter: str[i], state: ElementStates.Default }
  ...
  const reverse = async (arr: Array<TFibonacci>) => {
    let head: number = 0;
  ...
  const changeColor = (head: TFibonacci, tail: TFibonacci, state: string) => {
    if (state === 'default') {
